...
export default function DayViewBody({isLoading, listedTasks, setRefreshKey, setDisplayCreateTaskPopup}: DayViewBodyProps) {
    return (
        <VStack className="flex-1 items-center">
            ...
            {/*Loaded State*/}
            {!isLoading && listedTasks && listedTasks.length > 0 && (<ListViewBoxes listedTasks={listedTasks} setRefreshKey={setRefreshKey} />)}
            {!isLoading && listedTasks && listedTasks.length === 0 && (<Text className="text-2xl">Create Your First Task!</Text>)}
